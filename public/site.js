...
async function config(){try{const r=await fetch('/api/config');if(!r.ok)throw 0;return await r.json()}catch{return{whatsapp:''}}}
...
let PRODUCTS=[],CONFIG={whatsapp:''};const grid=document.querySelector('#product-grid'),count=document.querySelector('#product-count'),modal=document.querySelector('#product-modal'),modalContent=document.querySelector('#modal-content');
function esc(v=''){return String(v).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]))}
function wa(number,text){const d=(number||'').replace(/\D/g,'');return d?`https://wa.me/${d}?text=${encodeURIComponent(text)}`:'#'}
...
async function init(){try{const[pr,cr]=await Promise.all([fetch('/api/products'),fetch('/api/config')]);if(!pr.ok)throw 0;const pd=await pr.json();CONFIG=cr.ok?await cr.json():CONFIG;PRODUCTS=pd.products||[];render()}catch{grid.innerHTML='<p>Il catalogo sarà disponibile appena collegherai il database Cloudflare D1.</p>';count.textContent='Catalogo non collegato'}}
